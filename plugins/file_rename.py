...
async def process_file(client, message, media_type, file_id, file_name, file_size):
    ...
    semaphore = user_semaphores.setdefault(user_id, asyncio.Semaphore(4))
...
async def auto_rename_handler(client, message):
    ...
    if len(user_tasks[user_id]) >= 8:
        return await message.reply_text("Too many tasks queued. Wait for others to complete.")
